...
async function autoSeedDatabase() {
    try {
        ...
        const instructor = await db_1.default.user.upsert({
        ...
        const student = await db_1.default.user.upsert({
        ...
        for (const cat of categoryNames) {
            const createdCat = await db_1.default.category.upsert({
                where: { slug: cat.slug },
                update: {},
                create: cat,
            });
        ...
        if (courseCount === 0) {
            ...
            const k8sCourse = await db_1.default.course.create({
                data: {
            ...
            const dockerCourse = await db_1.default.course.create({
                data: {
            ...
            const sysDesignCourse = await db_1.default.course.create({
                data: {
            ...
            const k8sEnrollment = await db_1.default.enrollment.create({
                data: {
                    userId: student.id,
                    courseId: k8sCourse.id,
                    progress: 50,
            ...
            const k8sLessons = await db_1.default.lesson.findMany({
                where: { section: { courseId: k8sCourse.id } },
                orderBy: { sortOrder: 'asc' },
            });
            if (k8sLessons.length > 0) {
                await db_1.default.lessonProgress.create({
                    data: {
