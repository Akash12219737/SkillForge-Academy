...
exports.getNote = exports.saveNote = exports.getCertificates = exports.claimCertificate = exports.addReview = exports.getWishlist = exports.toggleWishlist = exports.updateLessonProgress = exports.getEnrolledCourseContent = exports.getEnrolledCourses = exports.enrollCourse = void 0;
...
const saveNoteToDisk = (userId, lessonId, content) => {
    ensureNotesStoreExists();
...
const retrieveNoteFromDisk = (userId, lessonId) => {
    ensureNotesStoreExists();
    const notes = getNotes();
    const key = `${userId}:${lessonId}`;
...
const getEnrolledCourseContent = async (req, res) => {
    try {
        ...
        const completedProgresses = await db_1.default.lessonProgress.findMany({
            where: {
...
exports.getEnrolledCourseContent = getEnrolledCourseContent;
...
const updateLessonProgress = async (req, res) => {
    try {
        ...
        await db_1.default.lessonProgress.upsert({
            where: {
        ...
        const allCourseLessons = await db_1.default.lesson.findMany({
            where: { section: { courseId } },
            select: { id: true },
        });
        const completedCount = await db_1.default.lessonProgress.count({
            where: {
                enrollmentId: enrollment.id,
                completed: true,
            },
        ...
        await db_1.default.enrollment.update({
...
const toggleWishlist = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const { courseId } = req.params;
        const existing = await db_1.default.wishlist.findUnique({
            where: { userId_courseId: { userId: req.user.id, courseId } },
        });
        if (existing) {
            await db_1.default.wishlist.delete({ where: { id: existing.id } });
...
const addReview = async (req, res) => {
    try {
        ...
        const enrolled = await db_1.default.enrollment.findUnique({
            where: { userId_courseId: { userId: req.user.id, courseId } },
        });
        if (!enrolled) {
...
const claimCertificate = async (req, res) => {
    try {
        ...
        const existing = await db_1.default.certificate.findFirst({
...
const getCertificates = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const certificates = await db_1.default.certificate.findMany({
            where: { userId: req.user.id },
            include: {
                course: {
